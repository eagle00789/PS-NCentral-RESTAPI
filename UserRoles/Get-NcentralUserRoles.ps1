...
<#
.SYNOPSIS
Get a list of all N-Central user Roles created under a specific Organisation ID

.DESCRIPTION
This function gets a list of user roles for a given OrganisationID

.PARAMETER orgUnitID
Optional. The Organisation ID. Defaults to 50 if not specified.

.PARAMETER UserRoleID
Optional. If specified, it will get only that role
...
.PARAMETER PageSize
Optional. Specifies the number of user roles to retrieve per page. Defaults to 50 if not specified.
...
.EXAMPLE
Get-NcentralUserRoles -orgUnitID 50

This example fetches all N-Central user roles for a customer with ID 50
...
#>
    ...
    param(
        ...
        [Parameter(Mandatory = $false, ParameterSetName = 'UserRole')]
        [int]$UserRoleID,
    ...
    switch ($PsCmdlet.ParameterSetName) {
        ...
        'UserRole' {
            $uri = "$script:BaseUrl/api/org-units/$orgUnitID/user-roles/$UserRoleID"

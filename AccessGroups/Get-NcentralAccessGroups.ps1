function Get-NcentralAccessGroups {
<#
.SYNOPSIS
Get a list of all N-Central Access Groups created under a specific Organisation ID

.DESCRIPTION
This function gets a list of Access Groups for a given OrganisationID
...
.PARAMETER PageSize
Optional. Specifies the number of Access Groups to retrieve per page. Defaults to 50 if not specified.

.PARAMETER All
Optional. If specified, retrieves all Access Groups across all pages
...
#>
    ...
    switch ($PsCmdlet.ParameterSetName) {
        'All' {
            $uri = "$script:BaseUrl/api/org-units/$orgUnitID/access-groups?pageNumber=$PageNumber&pageSize=$PageSize"
            if ($PSBoundParameters.ContainsKey('SortOrder')) {

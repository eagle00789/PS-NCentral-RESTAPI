...
<#
.SYNOPSIS
Get a list of all N-Central Customers

.DESCRIPTION
This function gets a list of all N-Central Customers

.PARAMETER PageNumber
Optional. Specifies which page of results to retrieve. Used when the total number of customers exceeds the page size. Defaults to 1 if not specified

.PARAMETER PageSize
Optional. Specifies the number of Customers to retrieve per page. Defaults to 50 if not specified.
...
.EXAMPLE
Get-NcentralCustomers -All

This example fetches all N-Central Customers
...
#>
    ...
    switch ($PsCmdlet.ParameterSetName) {
        'All' {
            $uri = "$script:BaseUrl/api/customers?pageNumber=$PageNumber&pageSize=$PageSize"
            if ($PSBoundParameters.ContainsKey('SortOrder')) {
